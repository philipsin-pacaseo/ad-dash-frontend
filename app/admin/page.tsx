      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div>
            ...
            <p className="text-gray-500">Super Admin Dashboard (雙引擎版)</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <h2 className="text-xl font-bold text-gray-800 mb-4">跨平台授權連結配發</h2>
              ...
              {tenants.map(tenant => (
                <div key={tenant.id} className="border border-gray-200 bg-gray-50 p-4 rounded-xl mb-4">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    <p className="text-sm font-bold text-blue-700 mb-2">🔵 Google Ads 授權連結：</p>
                    ...
                    <p className="text-sm font-bold text-indigo-700 mb-2">🟣 Meta (FB/IG) 授權連結：</p>
                    <div className="flex gap-2">
                      <input readOnly value={`${API_BASE_URL}/api/auth/meta/login?tenant_id=${tenant.id}&secret=${secret}`} className="text-xs w-full px-2 py-1 border border-gray-300 rounded bg-white text-gray-600" />
                      <button onClick={() => navigator.clipboard.writeText(`${API_BASE_URL}/api/auth/meta/login?tenant_id=${tenant.id}&secret=${secret}`)} className="bg-indigo-600 text-white text-xs px-3 py-1 rounded hover:bg-indigo-700">複製</button>
                    </div>
                  </div>
                </div>
              ))}
