...
export default function DashboardPage() {
  ...
  useEffect(() => { if (startDate && endDate) fetchDashboardData(startDate, endDate); }, [startDate, endDate]);
  ...
  const handleSort = (table: string, key: string) => {
    ...
    if (sortConfig && sortConfig.table === table && sortConfig.key === key && sortConfig.direction === 'desc') {
      direction = 'asc';
    }
    ...
    <div className="min-h-screen bg-gray-100 relative">
      ...
      <header className="bg-white shadow-sm px-8 py-4 flex flex-col xl:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-md border border-gray-200">
          ...
          <button onClick={() => fetchDashboardData()} className="ml-2 bg-blue-600 text-white px-3 py-1 rounded text-sm hover:bg-blue-700 transition">套用</button>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => { sessionStorage.clear(); router.push("/"); }} className="text-sm px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition">登出</button>
        </div>
      </header>
      ...
      <main className="p-8 max-w-7xl mx-auto space-y-8">
        ...
        {/* 🌟 雙 Y 軸趨勢圖 (花費 vs ROAS) */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h2 className="text-lg font-bold text-gray-800 mb-4">廣告花費與 ROAS 趨勢 ({curr})</h2>
          <div className="w-full h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data?.trend_chart || []} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
                <XAxis dataKey="date" stroke="#6b7280" fontSize={12} tickMargin={10} />
                
                {/* 左 Y 軸：總花費 */}
                <YAxis yAxisId="left" stroke="#6b7280" fontSize={12} tickFormatter={(v: any) => formatNum(v)} />
                {/* 右 Y 軸：ROAS */}
                <YAxis yAxisId="right" orientation="right" stroke="#ff7300" fontSize={12} tickFormatter={(v: any) => `${v}x`} />
                
                <Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                <Legend iconType="circle" wrapperStyle={{ paddingTop: '20px' }} />
                <Line yAxisId="left" type="monotone" dataKey="Total" name="總花費" stroke="#10b981" strokeWidth={3} dot={false} />
                <Line yAxisId="right" type="monotone" dataKey="ROAS" name="ROAS" stroke="#ff7300" strokeWidth={3} strokeDasharray="5 5" dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
